		...
		if(p_QGraphicsRectItemSelectionDash)
		{
			...
			for(int iF = 0; iF != lp_QGraphicsItems.count(); iF++)
			{
				...
				if(p_GraphicsItem->data(SCH_TYPE_OF_ITEM) == SCH_TYPE_ITEM_UI)
				{
					...
					{
						p_GraphicsElementItem = static_cast<GraphicsElementItem*>(p_GraphicsItem);
					}
					else if(p_GraphicsItem->data(SCH_KIND_OF_ITEM) == SCH_KIND_ITEM_GROUP)
					{
						p_GraphicsGroupItem = static_cast<GraphicsGroupItem*>(p_GraphicsItem);
					}
				}
...
void SchematicView::DeleteGroupRecursiveAPFS(GraphicsGroupItem* p_GraphicsGroupItem, bool bInitial)
{
	...
	{
		GraphicsGroupItem* p_GraphicsGroupItemHelper = p_GraphicsGroupItem->vp_ConnectedGroups.at(0);
		//
...
void SchematicView::DisbandSelectedGroupsAPFS()
{
	...
	{
		GraphicsGroupItem* p_GraphicsGroupItem = vp_SelectedGroupsMem.at(iF);
		//
...
void SchematicView::DetachSelectedAPFS()
{
	...
	for(int iG = 0; iG != vp_AffectedGroups.count(); iG++)
	{
		...
		GraphicsGroupItem* p_GraphicsGroupItemRoot =
				GetRootOfGroup(p_GraphicsGroupItem); // Взятие корня, вкл. текщую группу (в отличие от варианта в удалении).
...
bool SchematicView::ReplaceLink(GraphicsLinkItem* p_GraphicsLinkItem,
								GraphicsElementItem* p_GraphicsElementItem, bool bIsSrc, DbPoint& a_oDbPortPos, bool bFromElement)
{
	...
	if(bIsSrc)
	{
		...
		//
		oPSchLinkBase.oPSchLinkVars.oSchLGraph.oDbSrcPortGraphPos = BindToEdge(p_GraphicsElementItem, a_oDbPortPos);
	...
	else
	{
		...
		oPSchLinkBase.oPSchLinkVars.oSchLGraph.oDbDstPortGraphPos = BindToEdge(p_GraphicsElementItem, a_oDbPortPos);
	}
											...
											GraphicsLinkItem* p_GraphicsLinkItem, bool bIsIncoming)
{
	...
	{
		double dbDiameterLast = p_GraphicsElementItem->oPSchElementBaseInt.oPSchElementVars.oSchEGGraph.oDbFrame.dbW;
		//
		if(!bIsIncoming)
		{
			p_GraphicsElementItem->oPSchElementBaseInt.oPSchElementVars.oSchEGGraph.oDbFrame.dbW +=
		...
		for(int iC = 0; iC < iCn; iC++)
		{
			...
			if((p_GraphicsItem->data(SCH_TYPE_OF_ITEM) == SCH_TYPE_ITEM_UI) && (p_GraphicsItem->data(SCH_KIND_OF_ITEM) == SCH_KIND_ITEM_PORT))
			{
				...
				p_GraphicsPortItemInt = static_cast<GraphicsPortItem*>(p_GraphicsItem);
		...
		for(int iC = 0; iC < iCn; iC++)
		{
			...
			if(p_GraphicsItem->data(SCH_TYPE_OF_ITEM) == SCH_TYPE_ITEM_UI)
			{
				if(p_GraphicsItem->data(SCH_KIND_OF_ITEM) == SCH_KIND_ITEM_PORT)
				{
					...
					{
						PSchLinkVars* p_SchLinkVars = p_GraphicsPortItemInt->p_PSchLinkVarsInt;
						//
...
void SchematicView::RemovePortsByID(unsigned long long ullID)
{
	...
	{
		GraphicsPortItem* p_GraphicsPortItem = SchematicWindow::vp_Ports.at(iP);
		//
													 ...
													 bool bSendElements, bool bToTop, int iGroupsToGroupsGenerationsBeforeExclude)
{
	...
	{
		const EGPointersVariant* p_EGPointersVariant = &v_EGPointersVariants.at(iF);
		//
		if(p_EGPointersVariant->p_GraphicsElementItem != nullptr)
		{
...
void SchematicView::ElementMinOperations(QVector<GraphicsLinkItem*>& avp_GraphicsLinkItemCollection,
	...
	for(int iC = 0; iC < lp_Items.count(); iC++)
	{
		...
		if(p_GraphicsItem->data(SCH_TYPE_OF_ITEM) == SCH_TYPE_ITEM_UI)
		{
			if(p_GraphicsItem->data(SCH_KIND_OF_ITEM) == SCH_KIND_ITEM_PORT)
			{
				...
				if(!avp_GraphicsLinkItemCollection.contains(p_GraphicsPortItemInt->p_GraphicsLinkItemInt))
					avp_GraphicsLinkItemCollection.append(p_GraphicsPortItemInt->p_GraphicsLinkItemInt);
			}
		}
	}
...
void SchematicView::SetLinksVisAfterMinChanges(QVector<GraphicsLinkItem*>& avp_GraphicsLinkItemCollection)
{
	QVector<GraphicsLinkItem*> vp_GraphicsLinkItemUniques;
	//
	for(int iF = 0; iF != avp_GraphicsLinkItemCollection.count(); iF++)
	{
		GraphicsLinkItem* p_GraphicsLinkItem = avp_GraphicsLinkItemCollection.at(iF);
		//
...
void SchematicView::ElementMousePressEventHandler(GraphicsElementItem* p_GraphicsElementItem, QGraphicsSceneMouseEvent* p_Event)
{
	...
	if(DoubleButtonsPressControl(p_Event)) // Переключение минимизации.
	{
		...
		QVector<GraphicsLinkItem*> vp_GraphicsLinkItemCollection;
		//
...
void SchematicView::ElementMouseMoveEventHandler(GraphicsElementItem* p_GraphicsElementItem, QGraphicsSceneMouseEvent* p_Event)
{
	...
	{
		int iC = SchematicWindow::vp_SelectedElements.count();
		//
		for(int iE = 0; iE != iC; iE ++)
		{
...
void SchematicView::CreateGroupFromSelected()
{
	...
	for(int iF = 0; iF != SchematicWindow::vp_SelectedGroups.count(); iF++)
	{
		GraphicsGroupItem* p_GraphicsGroupItemUtil = SchematicWindow::vp_SelectedGroups.at(iF);
		//
	...
	for(int iF = 0; iF != SchematicWindow::vp_SelectedElements.count(); iF++)
	{
		GraphicsElementItem* p_GraphicsElementItemUtil = SchematicWindow::vp_SelectedElements.at(iF);
		//
...
void SchematicView::CBElementChanges(ExtElementData& a_oExtElementData)
{
	PSchData oSchData;
	unsigned int uiOverallSize = sizeof(PSchData) + sizeof(ExtElementData) + a_oExtElementData.uiDataSize; // Размер всех трёх составляющих.
	...
	memcpy(p_chBP, &a_oExtElementData, sizeof(ExtElementData));
	p_chBP += sizeof(ExtElementData);
	memcpy(p_chBP, a_oExtElementData.p_vData, a_oExtElementData.uiDataSize);
...
bool SchematicView::CheckLinkPresent(QVector<GraphicsLinkItem*>& avp_GraphicsLinkItemUniques, GraphicsLinkItem* p_GraphicsLinkItem)
{
	for(int iF = 0; iF != avp_GraphicsLinkItemUniques.count(); iF++)
	{
		GraphicsLinkItem* p_GraphicsLinkItemAtVector = avp_GraphicsLinkItemUniques.at(iF);
		if(p_GraphicsLinkItemAtVector == p_GraphicsLinkItem)
		{
			return true;
		}
		//
		double dbUSrcX = p_GraphicsLinkItemAtVector->p_GraphicsPortItemSrc->pos().x() +
						 p_GraphicsLinkItemAtVector->p_GraphicsPortItemSrc->p_ParentInt->pos().x();
		double dbUSrcY = p_GraphicsLinkItemAtVector->p_GraphicsPortItemSrc->pos().y() +
						 p_GraphicsLinkItemAtVector->p_GraphicsPortItemSrc->p_ParentInt->pos().y();
		double dbCSrcX = p_GraphicsLinkItem->p_GraphicsPortItemSrc->pos().x() +
						 p_GraphicsLinkItem->p_GraphicsPortItemSrc->p_ParentInt->pos().x();
		double dbCSrcY = p_GraphicsLinkItem->p_GraphicsPortItemSrc->pos().y() +
						 p_GraphicsLinkItem->p_GraphicsPortItemSrc->p_ParentInt->pos().y();
		//
		double dbUDstX = p_GraphicsLinkItemAtVector->p_GraphicsPortItemDst->pos().x() +
						 p_GraphicsLinkItemAtVector->p_GraphicsPortItemDst->p_ParentInt->pos().x();
		double dbUDstY = p_GraphicsLinkItemAtVector->p_GraphicsPortItemDst->pos().y() +
						 p_GraphicsLinkItemAtVector->p_GraphicsPortItemDst->p_ParentInt->pos().y();
		double dbCDstX = p_GraphicsLinkItem->p_GraphicsPortItemDst->pos().x() +
						 p_GraphicsLinkItem->p_GraphicsPortItemDst->p_ParentInt->pos().x();
		double dbCDstY = p_GraphicsLinkItem->p_GraphicsPortItemDst->pos().y() +
						 p_GraphicsLinkItem->p_GraphicsPortItemDst->p_ParentInt->pos().y();
		//
		if((dbUSrcX == dbCSrcX) && (dbUSrcY == dbCSrcY) && (dbUDstX == dbCDstX) && (dbUDstY == dbCDstY))
		{
			return true;
		}
		if((dbUSrcX == dbCDstX) && (dbUSrcY == dbCDstY) && (dbCSrcX == dbUDstX) && (dbCSrcY == dbUDstY))
		{
			return true;
		}
	}
...
void SchematicView::SetPortsPlacementAndLinksVisAfterGroupsMinChanges(QVector<GraphicsPortItem*>& avp_GraphicsPortItemsCollection)
{
	...
	for(int iF = 0; iF != avp_GraphicsPortItemsCollection.count(); iF++)
	{
		...
		{ // Если хоть один элемент из прикреплённых скрыт - добавляем уникальный линк в список для обработки.
			if(!vp_GraphicsLinkItemCollection.contains(p_GraphicsPortItemCurrent->p_GraphicsLinkItemInt)) // Елси уже не добавлен.
				vp_GraphicsLinkItemCollection.append(p_GraphicsPortItemCurrent->p_GraphicsLinkItemInt);
		}
	...
	for(int iE = 0; iE != p_GraphicsGroupItem->vp_ConnectedElements.count(); iE++)
	{
		...
		for(int iC = 0; iC < iCn; iC++)
		{
			...
			if(p_GraphicsItem->data(SCH_TYPE_OF_ITEM) == SCH_TYPE_ITEM_UI)
			{
				if(p_GraphicsItem->data(SCH_KIND_OF_ITEM) == SCH_KIND_ITEM_PORT)
				{
					GraphicsPortItem* p_GraphicsPortItemInt = static_cast<GraphicsPortItem*>(p_GraphicsItem);
					//
...
void SchematicView::LinkPaintHandler(GraphicsLinkItem* p_GraphicsLinkItem, QPainter* p_Painter)
{
	...
	if(!bLoading)
	{
		...
		{
			GraphicsElementItem* p_GraphicsElementItem = p_GraphicsLinkItem->p_GraphicsElementItemSrc;
			//
		...
		{
			GraphicsElementItem* p_GraphicsElementItem = p_GraphicsLinkItem->p_GraphicsElementItemDst;
...
bool SchematicView::PortMenuOperationsAPFS(int iData, GraphicsPortItem* p_GraphicsPortItem)
{
	PSchLinkBase oPSchLinkBase;
	int iNumber;
...
gDst:	iNumber = p_GraphicsPortItem->p_PSchLinkVarsInt->ushiDstPort;
	...
	else if(iData == MENU_SELECTED_PORT)
	{
		...
		if(bPortAltPressed) // Если жали Alt...
		{
			...
			for(int iF = 0; iF != lp_QGraphicsItems.count(); iF++)
			{
				...
				if(p_QGraphicsItem->data(SCH_TYPE_OF_ITEM) == SCH_TYPE_ITEM_UI)
				{
					...
					{
						//
						p_GraphicsElementItem = static_cast<GraphicsElementItem*>(p_QGraphicsItem);
						if(IsExtended(p_ElementSettings))
						{
					...
					{
						p_GraphicsGroupItem = static_cast<GraphicsGroupItem*>(p_QGraphicsItem);
			...
			if(p_QGraphicsItemFounded)
			{
				...
				{
					p_GraphicsElementItemFounded = static_cast<GraphicsElementItem*>(p_QGraphicsItemFounded); // Его в найденный рабочий.
				}
			}
...
gF:		ReleaseOccupiedAPFS();
	...
	else if((SchematicWindow::p_SafeMenu != nullptr) && bPortMenuReady)
	{
		...
		if(p_SelectedMenuItem != 0)
		{
			int iData = p_SelectedMenuItem->data().toInt();
			//
			if((iData == MENU_SRC_PORT) |
			   (iData == MENU_DST_PORT) |
			   (iData == MENU_SELECTED_PORT))
			{
				PortMenuOperationsAPFS(iData, p_GraphicsPortItem);
			}
			else if(iData == MENU_DELETE)
			{
